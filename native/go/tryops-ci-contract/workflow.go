...
func validateWorkflow(workflow string) []Check {
	...
	addContains("workflow_compose_and_native_contracts", []string{"docker compose config --quiet", "docker compose --profile tls config --quiet", "make native-container-contract-sample", "make native-dependency-lock-contract-sample", "make native-ci-contract-sample", "make evaluation-index-sample"})
