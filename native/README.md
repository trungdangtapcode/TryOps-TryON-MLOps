...
## Go Live Supply-Chain Evidence Gate

Path: `native/go/tryops-live-supply-chain`

Purpose:

- Dependency-free Go verifier for local live Syft/Trivy/Cosign execution evidence.
- Reads the Syft SPDX SBOM, Trivy filesystem JSON report, Cosign public key, signature, and
  `verify-blob` output generated by `make native-live-supply-chain-sample`.
- Fails production readiness on missing SBOM packages, any HIGH/CRITICAL Trivy vulnerability,
  misconfiguration, or secret finding, or failed Cosign blob verification.
- Emits `tryops.live_supply_chain.v1` for the Console evidence registry and native CI contract.

Source layout:

- `main.go`: thin CLI entrypoint.
- `config.go`: evidence path, output path, and pinned container image flags.
- `evaluate.go`: Syft, Trivy, and Cosign readiness checks.
- `jsonutil.go`, `files.go`, `types.go`: report parsing and JSON emission.
- `live_supply_chain_test.go`: pass/fail regression coverage.

Verified commands:

```bash
make native-live-supply-chain-test
make native-live-supply-chain-sample
```
...
- Cross-checks vulnerability, supply-chain, live supply-chain, and container-contract report schemas.
- Records local required-tool availability for Docker, Syft, Trivy, and Cosign; a passing
  `tryops.live_supply_chain.v1` report can satisfy Syft/Trivy/Cosign readiness without host PATH
  installs because the tools ran from pinned containers.
