...
- Native container image split: `artifacts/eval/containers/native_container_contract_report.json` must contain `tryops.native_container_contract.v1`, `passed=true`, seven required roles (`gateway`, `controller`, `guardrail`, `benchmark`, `cpp-tools`, `api`, `web-assets`), matching Compose services and Dockerfile contexts, multi-stage native build checks, non-SDK runtime checks for compiled images, source-path coverage, and Docker research/provenance links. `docker compose config` must also parse the updated Compose file.
