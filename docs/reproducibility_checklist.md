...
- Native secret rotation contract: `artifacts/eval/secrets/native_secret_rotation_contract.json` must contain `tryops.native_secret_rotation_contract.v1`, `passed=true`, 8 managed secrets, Vault KV provider details, hash-only API-key registry policy, Kubernetes workload identity with projected service-account token evidence, and External Secrets coverage for every managed secret. Plan evidence from `make native-secret-rotation-contract-sample` must contain `coverage_level=native_secret_rotation_plan_contract`, 50/50 passing plan checks, and `production_ready=false`. Live evidence from `make native-secret-rotation-live` must contain `coverage_level=native_secret_rotation_live_vault_kv_rotation`, `production_ready=true`, `summary.passed_checks=65`, `summary.live_kv_paths=5`, `summary.live_secret_rotations=8`, `live_readiness.auth_source=workload_identity_token_path`, `live_readiness.live_exercise_passed=true`, `live_readiness.vault_health_ok=true`, `live_readiness.secret_properties_rotated=8`, and `live_readiness.max_version_observed >= 2`, without raw Vault tokens or generated secret values in the report. Production cluster readiness still requires exercising the External Secrets controller against Kubernetes.
...
- Native distributed quota admission: `artifacts/eval/quota/native_distributed_quota_admission.json` must contain `tryops.distributed_quota_admission.v1`, `passed=true`, `coverage_level=native_rust_postgres_distributed_quota_admission`, two Rust gateway URLs, `summary.allowed=20`, `summary.rejected=12`, `summary.errors=0`, `checks.no_cluster_quota_oversell=true`, and `checks.allowed_equals_global_limit=true`. `make native-distributed-quota-smoke` must use a disposable Compose Postgres project and `TRYOPS_GATEWAY_QUOTA_POSTGRES_ADMISSION=true`.
...
- Live supply-chain scan/signature evidence: `artifacts/eval/ci/live_supply_chain_report.json` must contain `tryops.live_supply_chain.v1`, `passed=true`, `production_ready=true`, pinned Syft/Trivy/Cosign container image names, a non-empty SPDX SBOM package count, zero HIGH/CRITICAL Trivy vulnerability/misconfiguration/secret findings, a Cosign public key, a signature over the SBOM blob, and `verify-blob` output containing `Verified OK`.
- Native CI supply-chain contract: `artifacts/eval/ci/native_ci_contract.json` must contain `tryops.native_ci_contract.v1`, `passed=true`, workflow path `.github/workflows/ci.yml`, `make ci` coverage, GitHub Actions OIDC permissions, upload-artifact evidence, seven-image Docker Buildx matrix, Syft SPDX SBOM generation, Trivy HIGH/CRITICAL scan gate, Cosign keyless signing on non-PR pushes, and schema references to vulnerability, supply-chain, live supply-chain, and container-contract reports. If `production_ready=false`, the report must list missing required local tools and cannot be used as live Syft/Trivy/Cosign execution evidence.
- Evaluation index: `artifacts/eval/evaluation_index/evaluation_index.json` must contain `tryops.evaluation_index.v1`, report counts, highlighted Pareto/energy/VTON/native-VTON-API/drift/full-stack/demo/security/live-supply-chain/CI/dependency-lock/config/secrets/postgres/backup/tls/container/performance/fullstack-load/trace-envelope/observability/alertmanager/quota-read-model/distributed-quota/runtime-telemetry evidence, `pipeline_runs` from run-context/OpenLineage/lineage artifacts, `optimization_panel` with `recommended_variant="4bit"`, `carbon_gate_verdict="pass"`, per-variant energy and SCI metrics, live supply-chain evidence, native SLO gate evidence, native event-dispatcher evidence, native CI contract evidence, native dependency-lock evidence, native config-contract evidence, native secret-rotation evidence, native Postgres migration evidence, native backup/restore drill evidence, native TLS termination evidence, native container-contract evidence, native trace-envelope evidence, native full-stack load SLO evidence, native observability evidence, native Alertmanager evidence, native quota-read-model evidence, native distributed-quota evidence, native runtime-telemetry evidence, native performance-budget evidence, and the Console endpoint `/api/evaluations/summary` must pass through `make app-smoke`.
