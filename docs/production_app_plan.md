...
```
              Browser (enterprise users + admins)
                       │ HTTPS
          ┌────────────▼──────────────┐
          │ Keycloak IAM / OIDC login │
          │ realm import · PKCE       │
          └────────────┬──────────────┘
                       │ bearer JWT
          ┌────────────▼──────────────────────┐
          │ Rust Axum edge                     │
          │ OIDC/JWKS auth · quota/rate/body   │
...
- Keycloak provides open-source IAM with OIDC/OAuth2 flows for applications and services; the Console uses authorization-code + PKCE, the Rust gateway validates RS256 JWTs through JWKS, and local Compose imports the TryOps realm for deterministic development: https://www.keycloak.org/documentation and https://openid.net/specs/openid-connect-core-1_0.html
...
- **Rust:** edge gateway, OIDC/JWKS auth preflight, quota admission, semantic-cache admission plus C++ lookup invocation, rate limiting, payload limits, trace propagation, static-serving profile, low-latency proxy paths.
...
6. **Enterprise deployment profile.** Replace demo credentials with `.env.example` + secret loading, keep local Keycloak/OIDC wired for IAM, add Postgres migrations/pooling/backup/restore, TLS termination, Alertmanager, OpenTelemetry Collector, scanner/signing CI, dependency locks, and a product runbook.
...
- [~] **P3 — Frontend.** PARTIAL: React/Vite/TypeScript Console shell, left nav, typed API client, Keycloak OIDC login/signup/logout with PKCE token storage and refresh, local API-key demo fallback, LLM Playground with direct/canary/A-B/bandit routing modes, VTON Studio contract form plus persisted side-by-side comparison outputs, Dashboard with native quota/showback read model, Account Dashboard, Request History, Pipeline Runs, Model Registry, Evaluation evidence plus optimization/Pareto/sustainability panel, Experiments board, Governance, Incident posture plus live bad-candidate and rollback drills, native incident workflow/postmortem evidence loading, Professor Demo mode, RBAC role-aware navigation, degraded banner, and responsive styling are implemented under `web/`; browser upload/download controls, full audit-log UI, live alert workflow controls, screenshots, and accessibility/export hardening remain.
- [x] **P4 — Services & edge wiring.** DONE: modular Rust gateway reverse-proxies `/api/*` to FastAPI `/v1/*`, adds request IDs, propagates trace context, enforces edge limits/preflight plus artifact-path preflight, calls the modular Go guardrail sidecar before LLM generation, and `docker-compose` ties gateway+api+db+keycloak+prometheus+grafana+guardrail+minio+mlflow with healthchecks plus `make app-up`, `make app-smoke`, and `make app-down`.
...
- [x] PA035 Auth/login + session + role-aware nav. (Keycloak-backed OIDC login/signup/logout now uses authorization-code + PKCE in `web/src/auth.ts`, `/api/auth/config` returns provider endpoints, `/api/auth/session` returns the active RBAC principal and allowed Console navigation, the React shell hides views outside the active scopes, and a local API-key fallback remains for deterministic offline demos.)
...
- [x] PA041 `docker-compose`: gateway + api + db(postgres) + keycloak + prometheus + grafana + guardrail + minio. (`docker-compose.yml` now starts Keycloak 26 with a Postgres schema initializer and imports `configs/keycloak/tryops-realm.json`; `Dockerfile.gateway` builds the edge; `docker compose config` validates.)
...
- [x] PA044 `.env` config + secrets (no hardcoded creds). (`.env.example` documents required local secret variables including Keycloak port/admin bootstrap values, `.env` remains ignored, `docker-compose.yml` now uses Compose secrets for Postgres/MinIO/MLflow/gateway quota DSN paths, the Rust gateway reads `TRYOPS_GATEWAY_QUOTA_POSTGRES_DSN_FILE`, and `native/go/tryops-config-contract/` validates secrets, direct credential-env absence, and `.env.example` coverage via `make native-config-contract-sample`.)
...
- [x] PA055 RBAC roles: viewer / operator / admin enforced on routes + nav. (`configs/api_keys.json` still has active demo principals with `session:read`; `configs/keycloak/tryops-realm.json` defines platform/admin/account roles for OIDC users; FastAPI exposes `/api/auth/config`, `/api/auth/session`, account bootstrap/read-model endpoints, and `/v1/auth/session`; the Rust gateway protects `/v1/auth/session` before proxying and forwards non-secret principal metadata from validated JWTs; the Console filters nav by the returned permission set and promotion actions use the active session instead of a hard-coded privileged key.)
...
- [x] PA100 OIDC / SSO login (Keycloak) replacing API-key-only. (`docker-compose.yml` runs Keycloak 26 on `${TRYOPS_KEYCLOAK_PORT:-18082}` with Postgres-backed schema init and realm import from `configs/keycloak/tryops-realm.json`; FastAPI serves OIDC config and account bootstrap/read-model endpoints; the Console performs PKCE login/signup/logout/refresh; the Rust gateway validates Keycloak-issued RS256 JWTs through JWKS while retaining API-key fallback for offline demos.)
