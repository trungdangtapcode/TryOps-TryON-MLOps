...
- **Enterprise profile:** Docker Compose healthchecks/readiness, `.env.example` plus Compose secret loading, native Rust TLS termination with Compose certificate/key secrets, Prometheus/Grafana provisioning, OpenTelemetry Collector, native Postgres migration/pooling, backup/restore, MinIO object URLs, GitHub Actions CI, SBOM/vuln/signing gates with open-source tooling, and native CI contract evidence.
